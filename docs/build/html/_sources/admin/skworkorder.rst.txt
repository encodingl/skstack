 ...
 * sk__name 本工单名称
 * sk__env 本工单管理环境名称
 * sk__task_name_created 此变量有工单名称和工单提单时间组合而成，目前用于传递给 docker 插件用于日志文件尾缀，用于审计的日志详情
...
目前在 Docker 部署功能设计了日志详情，便于发现问题后进行跟踪查看日志。

.. image:: _images/history_log.png
   :alt: image not found

.. note::
	  #. 需要配合 Docker 插件模块一起使用方可。一个是在 web 的配置文件配置插件的日志地址，如果日志备份在地址下面的 bak 目录也可以。第二是要配置主任务时添加 -tnc {sk__task_name_created}
