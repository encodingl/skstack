...
from .forms import WorkOrderFlow_detail_form,WorkOrderFlow_release_form,WorkOrderFlow_schedule_detail_form,WorkOrderFlow_schedule_detail_form2
...
@login_required()
@permission_verify()
def WorkOrderFlow_schedule(request):
    temp_name = "skworkorders/skworkorders-header.html"  
    current_date=timezone.now()  
    tpl_env = Environment.objects.all()
    tpl_dic_obj={}
...
#         tpl_all = WorkOrderFlow.objects.filter(created_at__range=(current_date + timedelta(days=-30),current_date),celery_task_id__isnull=False)
    ...
    return render_to_response('skworkorders/WorkOrderFlow_schedule.html', locals(), RequestContext(request))
...
#     temp_name = "skworkorders/skworkorders-header.html"
    ...
    if WorkOrderFlow_id:
        if obj.celery_task_id:
            t01 =  WorkOrdkerFlowTask(WorkOrderFlow_id)
...
@login_required()
@permission_verify()
def WorkOrderFlow_schedule_detail(request): 
    task_id = request.GET.get('task_id', '')  
    
    obj = get_object(TaskResult, task_id=task_id) 
    obj2 = get_object(WorkOrderFlow, celery_task_id=task_id)
    if not obj and obj2.status == "PENDING":
        return HttpResponse("任务未开始，请等待计划时间执行后再查看")
    elif not obj and obj2.status == "REVOKED":
        return HttpResponse("任务已撤销，无执行详情信息")
    else:
        obj=obj.as_dict()
        obj["result"] = obj["result"].decode("unicode_escape")
        tpl_WorkOrderFlow_form = WorkOrderFlow_schedule_detail_form2(initial=obj)
        return render_to_response("skworkorders/WorkOrderFlow_detail.html", locals(), RequestContext(request))
...
#     temp_name = "skworkorders/skworkorders-header.html"
    ...
    t01 = WorkOrdkerFlowTask(WorkOrderFlow_id,login_user)
    ...
    if obj_level == "1":
        ...
        if obj_l1 in obj_group:
            ...
            if obj_WorkOrderFlow.celery_schedule_time:
                t01.celery_task_add()
